(function() {
  ...
  function updateProjectTypeButtons() {
    try {
      ...
      const disabled = settings.disabledProjectTypes || [];
    ...
    } catch (e) { console.error('Failed to update project type buttons:', e); }
  }
  ...
  function toggleProjectType(type) {
    try {
      ...
      const idx = settings.disabledProjectTypes.indexOf(type);
  ...
  function removeBlockedProject(id) {
    try {
      ...
      const idx = settings.blockedProjects.indexOf(id);
  ...
  function updateBlockedProjectsList() {
    ...
    try {
      const settings = JSON.parse(document.getElementById('jsonEditor').value);
      const list = settings.blockedProjects || [];
    ...
    } catch (e) { console.error('Failed to update blocked projects list:', e); }
  }
  ...
  function updateEasyUI(s) {
    const ew = document.getElementById('enableWarning');
  ...
  function resetToDefaults() {
    const d = { modrinthInstallationWarning: false, warningTitle: 'Installation Temporarily Disabled', warningMessage: 'Installations are temporarily disabled.', disabledProjectTypes: [], blockedProjects: [] };
